...
### This is nearly exactly the same as the query_gitlab_contributors method
    def query_gitee_contribtutors(self, entry_info, repo_id):

        gitlab_url = entry_info['given']['gitee_url'] if 'gitee_url' in entry_info['given'] else entry_info['given']['git_url']
        ...
        for repo_contributor in contributors:
            try:
                ...
                r = requests.get(url=cntrb_url, headers=self.headers)
                contributor = r.json()
### This mapping should be abstracted so that each platform's mapping assigns a variable value
                cntrb = {
                    "cntrb_login": contributor.get('username', None),
                    "cntrb_created_at": contributor.get('created_at', None),
                    "cntrb_email": email,
